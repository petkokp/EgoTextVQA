...
def print_score(model):
    ...
    for sample in new_pred_contents:
        ...
        if existing_data:
            exists = any(item['question_id'] == question_id and item['video_id'] == video_id for item in existing_data)
            if exists:
                with open('log.txt', 'w') as log_file:
...
def calculate_score(model_name, dataset):
    ...
    for sample in tqdm(output_contents, desc="Evaluating predictions"):
        ...
        if question_type not in score_data:
            continue
...
if __name__ == "__main__":
    ...
    dataset = 'indoor'
    print("Calculating scores for outdoor dataset:")
    calculate_score(model_name, dataset)
    dataset = 'outdoor'
    print("Calculating scores for indoor dataset:")
    calculate_score(model_name, dataset)